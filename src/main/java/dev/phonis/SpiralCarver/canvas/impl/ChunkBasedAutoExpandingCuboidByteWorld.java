...
public class ChunkBasedAutoExpandingCuboidByteWorld implements CuboidByteWorld
{
    ...
    public ChunkBasedAutoExpandingCuboidByteWorld()
    {
    ...
    private int getOffsetFor(int pos)
    {
        return Math.floorMod(pos, ChunkBasedAutoExpandingCuboidByteWorld.CHUNK_SIZE);
    }

    private ChunkPos getChunkPosFor(int x, int y, int z)
    {
        return new ChunkPos(
    ...
    private byte[][][] getOrCreateChunkFor(ChunkPos chunkPos)
    {
    ...
    private void expandLogicalBounds(int x, int y, int z)
    {
