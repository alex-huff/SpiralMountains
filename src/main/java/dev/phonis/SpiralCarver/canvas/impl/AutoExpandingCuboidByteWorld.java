...
public
class AutoExpandingCuboidByteWorld implements CuboidByteWorld
{
	...
	void expandY(int y)
	{
		if (this.numInBounds(y, this.logicalBounds.getMinY(), this.logicalBounds.getMaxY()))
		{
			return;
		}
	...
	void expandZ(int z)
	{
		if (this.numInBounds(z, this.logicalBounds.getMinZ(), this.logicalBounds.getMaxZ()))
		{
			return;
		}
	...
	private
	boolean numInBounds(int i, int min, int max)
	{
		return i >= min && i <= max;
	}
